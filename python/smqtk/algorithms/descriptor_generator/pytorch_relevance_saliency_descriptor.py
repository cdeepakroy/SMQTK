...
class RelevanceMaskSaliencyDataset(data.Dataset):
    ...
    def rel_index(self):
        """
        :return: instance of the RelevancyIndex class used to compute the relevance of a descriptor
        :rtype: smqtk.algorithms.RelevancyIndex
        """
        return self._rel_index

    @rel_index.setter
    def rel_index(self, val):
        """
        set the RelevancyIndex class that can be used to compute the relevance of a descriptor

        :param val: instance of the RelevancyIndex class used to compute the relevance of a descriptor
        :type val: smqtk.algorithms.RelevancyIndex
        """
        self._rel_index = val
    ...
    def __getitem__(self, index):
        ...
        if self._rel_index is None:
            ...
            # compute distance of unmasked image with query image
        ...
        def obtain_masked_img_targetP(img):
            ...
            for m_img in tqdm(masked_imgs_loader, total=len(masked_imgs_loader), desc='Predicting masked images'):
                ...
                masked_img_f = self._classifier(m_img)[0]

                if self._rel_index is None:

                    # compute distance of masked image with query image
                    if self._dis_type is DIS_TYPE.L2:
                        masked_img_dis = L2_dis(query_f, masked_img_f, dim=1)
                    elif self._dis_type is DIS_TYPE.hik:
                        masked_img_dis = his_intersection_dis(query_f, masked_img_f, dim=1)
                    else:
                        raise TypeError("Unsupport distance type {}".format(
                            self._dis_type))

                    dis_diff = masked_img_dis - org_dis
...
class PytorchRelevanceSaliencyDescriptorGenerator (DescriptorGenerator):
    """
    ...
    def __init__(self, model_cls_name, model_uri=None, grid_size=20, stride=4,
        ...
        """
        super(PytorchRelevanceSaliencyDescriptorGenerator, self).__init__()
    ...
    def _setup_network(self):
        ...
        self.saliency_generator = RelevanceMaskSaliencyDataset(masks, self.model_cls, self.batch_size,
    ...
    def compute_descriptor(self, data, descr_factory=DFLT_DESCRIPTOR_FACTORY,
        ...
        :param rel_index: Instance of a RelevancyIndex derived class
        :type rel_index: smqtk.algorithms.RelevancyIndex
        ...
        """
        m = self.compute_descriptor_async(
            [data], descr_factory, overwrite,
        ...
        if uuid4proc:
            ...
            for (d, uuids, resized_org_img, w, h, gt_labels) in tqdm(data_loader, total=len(data_loader),
                ...
                if saliency_flag:
                    ...
                    self.saliency_generator.rel_index = rel_index
